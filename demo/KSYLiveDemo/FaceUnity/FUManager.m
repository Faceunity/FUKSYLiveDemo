...
- (CVPixelBufferRef)renderItemsToPixelBuffer:(CVPixelBufferRef)pixelBuffer
{
    ...
    CVPixelBufferRef buffer = [[FURenderer shareRenderer] renderPixelBuffer:pixelBuffer withFrameId:frameID items:items itemCount:sizeof(items)/sizeof(int) flipx:isFrontCamera];//flipx 参数设为YES可以使道具做水平方向的镜像翻转
    frameID += 1;
    
    return buffer;
}

- (void)renderItemsWithWidth:(int)w Height:(int)h Data:(void *)bytes {
    
    [self setBeautyParams];
    
    fuRenderItemsEx(FU_FORMAT_BGRA_BUFFER, bytes, FU_FORMAT_BGRA_BUFFER, bytes, w, h, frameID, items, 3);
    frameID ++ ;
}
